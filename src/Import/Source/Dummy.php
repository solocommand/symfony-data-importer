...
/**
 * Generates dummy data for import testing
 */
class Dummy extends Source
{
    ...
    {
        $this->li = new LoremIpsum();
    }

    /**
     * {@inheritdoc}
     */
    public function count($from, $criteria)
    {
     ...
     * {@inheritdoc}
     */
    public function retrieve($from, $criteria, array $fields = [], array $sort = [], $limit = 200, $skip = 0)
    {
        $docs = [];
        for ($i=0; $i < $limit; $i++) {
            $docs[] = $this->createDoc();
    ...
    private function createDoc()
    {
        return [
            'id'        => $this->id++,
            'name'      => $this->li->words(mt_rand(3, 10)),
            'body'      => $this->li->paragraphs(mt_rand(1,15), 'p'),
            'status'    => 1
        ];
    }
}
